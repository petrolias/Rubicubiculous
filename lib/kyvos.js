var Kyvos = {
    ...
    Main: function () {
        ...
        this.theme = Kyvos.Theme.getDeafaultTheme();
        this.cube = Kyvos.Cube.getDefaultCube();

        var context = this.context;
        var theme = this.theme;
        var cube = this.cube;
        var thumbnailCube = Kyvos.Cube.getThumbnailCube();

        this._adjustCubeCanvasSize(context, cube);
        this.Public.CubeRects = this._refreshContext(context, theme, cube);
        //this.Events.onClick(this.Public.CubeRects, this._const.getCanvas());
    ...
    _drawCase: function (alg, context, theme, cube) {
        alg.inner.forEach(f => {
            var x = f[0];
            var y = f[1];
    ...
    _renderCase: function (context, theme, cube, alg) {
        this.Public.CurrentCase = alg;
        this._refreshContext(context, theme, cube);
        this._drawCase(context, theme, cube, alg);
    ...
    renderCase : function(){
        ...
        var theme = this.theme;
        var cube = this.cube;
        this._renderCase(context, theme, cube, alg);
    },
    refreshCurrentCase: function () {
        this._renderCase(this.Public.CurrentCase);
    },
    _logAlg: function (alg) {
        //console.log(alg);
        var json = JSON.stringify(alg, null);
        //console.log(json);
        this._setAlgorithm(json);
    },
    logCurrentCase: function () {
        this._logAlg(this.Public.CurrentCase);
    },
    ...
    _generateCases: function (theme, cube) {        
        ...
        Kyvos.Cases.sort((a, b) => (a.gn > b.gn) ? 1 : -1).forEach(a => {
