var Kyvos = {
    ...
    _const: {
        //normalQube: { w : 100, smallQubeAnalogy:0.2, offset : 5 , padding: 1, startPositionOffeset: 0},
        normalQube: { w : 30, smallQubeAnalogy:0.2, offset : 1 , padding: 1, startPositionOffeset: 0},
        ...
        colors: { background: "lightGray", topPhase: "yellow", border: "black", outerCubeInner : "white", outerCubeBorder:"lightGray"}
    },
    _adjustCubeCanvasSize : function(){
        var canvas = this._const.getCanvas();
        var nq = this._const.GetNormalQube();
        canvas.width = nq.getWidth();
        canvas.height = canvas.width;
    },
    _drawCubeSceleton: function (theme) {
        var color = this._const.colors.background;
        var size = this._const.GetNormalQube().size;
        var rects = [];
        for (i = 0; i < size; i++) {
            for (j = 0; j < size; j++) {
                var rect = this._drawQube(i, j, theme);
                rects.push(rect);
            }
            ["t", "b", "l", "r"].forEach(e => {
                var rect = this._drawSmallQube(i, e, theme);
                rects.push(rect);
            });
        }
        return rects;
    },
    _drawQube: function (i, j, theme) {
        var nq = this._const.GetNormalQube();
        var w = nq.w;
        var h = nq.h;
        var pd = nq.padding;
    ...
    _drawSmallQube: function (i, direction, theme) {
        var nq = this._const.GetNormalQube();
        var w = nq.w;
        var h = nq.sqh;
        var pd = nq.padding;
        ...
        var ctx = this._const.getContext();
        ctx.fillStyle = theme.outerCubeBorder;
        ctx.fillRect(x, y, ww, hh);
        ctx.fillStyle = theme.outerCubeInner;
